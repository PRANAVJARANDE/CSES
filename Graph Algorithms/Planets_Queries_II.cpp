...
void helper(ll in,vll&dis,vll&vis,vll&a)
{
    if(vis[in])return;
    vis[in]=1;
    helper(a[in],dis,vis,a);
    dis[in]=dis[a[in]]+1;
}

ll jump(ll a,ll dist,vvl&up)
{
    if(dist<0)return -1;
    for(int i=0;i<31;i++)
    {
...
void solve()
{
    ...
    vll vis(n,0);
    vll dis(n,0);
    for(int i=0;i<n;i++)if(vis[i]==0)helper(i,dis,vis,a);
    ...
    while(q--)
    {
        ...
        b--;
        ll ans=-1;
        ll entry=jump(a,dis[a],up);
        if(jump(a,dis[a]-dis[b],up)==b)ans=dis[a]-dis[b];
        else if(jump(entry,dis[entry]-dis[b],up)==b)ans=dis[entry]-dis[b]+dis[a];
        cout<<ans<<endl;
    }
