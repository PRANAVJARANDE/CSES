...
void dfs(ll in,ll par,vvl &anc,vvl&adj,vll&d,ll l)
{
    anc[in][0]=par;
    d[in]=l;
    for(int i=1;i<=20;i++)
    ...
    for(ll x:adj[in])
    {
        if(x!=par)
        {
            dfs(x,in,anc,adj,d,l+1);
        }
    }
}


ll getlca(vll & d,ll x,ll y,vvl&anc)
{
    if(d[x]>d[y])swap(x,y);
        ll k=d[y]-d[x];
        for(int i=20;i>=0;i--)
...
void solve()
{
    ...
    while(q--)
    {
        ...
        x--;
        y--;
        ll lca=getlca(d,x,y,anc);
        ll ans=d[x]+d[y]-2*d[lca];
        cout<<ans<<endl;
    }    
}
