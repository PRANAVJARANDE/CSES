...
bool getbit(ll a,ll i)
{
    return (a & (1<<i));
}

void generatenext(ll mask,ll nx,ll in,vll&res)
{
    if(in>=n)
    {
        res.push_back(nx);
    ...
    if(getbit(mask,in))
    {
        generatenext(mask,nx,in+1,res);
        return;
    }
    if(in+1<n && !getbit(mask,in+1))generatenext(mask,nx,in+2,res);
    ll nh=nx | (1<<in);
    generatenext(mask,nh,in+1,res);
}

ll helper(ll in,ll mask,vector<vll>&dp)
{
    if(in>=m)
    {
        if(mask==0)return 1;
    ...
    vll res;
    generatenext(mask,0,0,res);
    ll ans=0;
    for(auto x:res)
    {
        ans=((ans%MOD) + (helper(in+1,x,dp))%MOD)%MOD;
    }
...
{
    cin>>n>>m;
    vector<vll>dp(1 << (n+1),vll(m,-1));
    ll ans=helper(0,0,dp);
    cout<<ans<<endl;
}
