...
ll n,m;
ll helper(ll i,ll j,vll&a,vll&b,vvl&dp)
{
    if(i>=n || j>=m)return 0;
    if(dp[i][j]!=-1)return dp[i][j];
    if(a[i]==b[j])return dp[i][j]=1+helper(i+1,j+1,a,b,dp);
    ll op1=helper(i+1,j,a,b,dp);
    ll op2=helper(i,j+1,a,b,dp);
...
void solve()
{
    ...
    while(i<n && j<m)
    {
        ...
        {
            ll op1=0,op2=0;
            if(i+1<n)op1=dp[i+1][j];
            if(j+1<m)op2=dp[i][j+1];
            if(op1>=op2)i++;
            else j++;
        }
    }
