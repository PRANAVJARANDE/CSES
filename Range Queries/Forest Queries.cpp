...
void solve()
{
    ...
    vector<vector<ll>>psum(n,vector<ll>(n,0));
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n;j++)
        {
            psum[i][j]=((i-1>=0)?psum[i-1][j]:0)+((j-1>=0)?psum[i][j-1]:0)-((i-1>=0 && j-1>=0)?psum[i-1][j-1]:0);
            if(a[i][j]=='*')psum[i][j]++;
        }
    }
    while(q--)
    {
        ll i,j,l,r;cin>>i>>j>>l>>r;
        i--;
        j--;
        l--;
        r--;
        ll ans=psum[l][r];
        if(i-1>=0)ans-=psum[i-1][r];
        if(j-1>=0)ans-=psum[l][j-1];
